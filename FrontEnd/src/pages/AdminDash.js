...
export const AdminDashboard = () => {
  ...
  useEffect(() => {
    const fetchUsersWithOrders = async () => {
      try {
        const response = await axios.get('http://13.127.47.145:3001/user/');
  ...
  const handleDeleteUser = async (userId) => {
    try {
      await axios.delete(`http://13.127.47.145:3001/user/${userId}`);
  ...
  const handleDeleteOrder = async (userId, orderId) => {
    try {
      await axios.delete(`http://13.127.47.145:3001/orders/savedorders/${userId}/${orderId}`);
